  ...
  return (
    <main className="bg-[#f5f5f7] px-5 py-20">
      ...
      <div className="mx-auto mt-10 grid max-w-7xl gap-5 md:grid-cols-3">
        {albums.length ? albums.map((album) => <GalleryCard key={album.id} album={album} />) : <div className="md:col-span-3"><EmptyState title="Belum ada album gallery" description="Album pelayanan akan tampil setelah dipublikasikan dari CMS." /></div>}
      </div>
    </main>
  );
}
