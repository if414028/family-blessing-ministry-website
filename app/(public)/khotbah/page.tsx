...
import { getPublishedSermons, type PublishedSermon } from "@/lib/data";
  ...
  return (
    <main className="bg-[#f5f5f7] px-5 py-20">
      ...
      <div className="mx-auto mt-10 grid max-w-7xl gap-5 md:grid-cols-3">
        {sermons.length ? sermons.map((sermon: PublishedSermon) => <SermonCard key={sermon.id} sermon={sermon} />) : <div className="md:col-span-3"><EmptyState title="Belum ada khotbah terpublikasi" description="Video khotbah akan tampil setelah ditambahkan dari CMS." /></div>}
      </div>
    </main>
  );
}
