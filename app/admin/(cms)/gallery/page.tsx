...
export default async function AdminGalleryPage() {
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-[18px] bg-white">{albums.map((album: AdminAlbum) => <div key={album.id} className="grid gap-3 border-b border-[#e0e0e0] p-4 md:grid-cols-[1fr_150px_130px_90px]"><div><p className="font-semibold">{album.title}</p><p className="text-sm text-[#7a7a7a]">{album.branch?.name ?? "General"}</p></div><span>{formatDate(album.date)}</span><StatusBadge>{album.status}</StatusBadge><Link className="text-[#0066cc]" href={`/admin/gallery/${album.id}/edit`}>Edit</Link></div>)}</div>
    </div>
  );
}
